...
var config = Config{
	...
	PromptZH2JA: `你是 Azure 技术文档的专业翻译助手，任务是将字符串中的【简体中文】翻译为【日文】，原文是中文，尽量在保留原意的前提下转换成日本人容易理解的表达, 并保持所有英文、数字、符号、Azure 技术词原样不变。
规则：
1. 只翻译中文；英文必须完全保持原样。
...
5. 输出必须是 JSON 对象：{"原文": "译文", ...}，使用原文字符串作为键，禁止任何额外说明。`,
	...
	PromptJA2ZH: `你是 Azure 技术文档的专业翻译助手，任务是将字符串中的【日文】翻译为【简体中文】，原文是日文，尽量在保留原意的前提下转换成中国人容易理解的表达, 并保持所有英文、数字、符号、Azure 技术词原样不变。
规则：
1. 只翻译日文；英文必须完全保持原样。
